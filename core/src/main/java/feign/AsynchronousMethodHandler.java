...
final class AsynchronousMethodHandler<C> implements MethodHandler {
    ...
    return client.execute(request, options, Optional.ofNullable(requestContext))
        ...
        .exceptionally(throwable -> {
          ...
          if (completionException.getCause() instanceof IOException) {
            ...
            if (methodHandlerConfiguration.getLogLevel() != Logger.Level.NONE) {
              methodHandlerConfiguration.getLogger().logIOException(methodHandlerConfiguration.getMetadata().configKey(), methodHandlerConfiguration.getLogLevel(), ioException,
                  elapsedTime(start));
            }
  ...
  private CompletableFuture<Object> handleResponse(Response response, long elapsedTime) {
    return asyncResponseHandler.handleResponse(
        methodHandlerConfiguration.getMetadata().configKey(), response, methodInfo.underlyingReturnType(), elapsedTime);
  }
