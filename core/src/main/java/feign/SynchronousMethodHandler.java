...
final class SynchronousMethodHandler implements MethodHandler {
    ...
    private MethodHandlerConfiguration methodHandlerConfiguration = new MethodHandlerConfiguration(null, null, null, null, null, null, null, null, null);
    ...
    this.methodHandlerConfiguration.setRequestInterceptors(checkNotNull(requestInterceptors, "requestInterceptors for %s", target));
    ...
    this.methodHandlerConfiguration.setBuildTemplateFromArgs(checkNotNull(buildTemplateFromArgs, "metadata for %s", target));
    ...
    while (true) {
      try {
        ...
        if (methodHandlerConfiguration.getLogLevel() != Logger.Level.NONE) {
          methodHandlerConfiguration.getLogger().logRetry(methodHandlerConfiguration.getMetadata().configKey(), methodHandlerConfiguration.getLogLevel());
        }
        continue;
      }
    }
  ...
  Object executeAndDecode(RequestTemplate template, Options options) throws Throwable {
    ...
    if (methodHandlerConfiguration.getLogLevel() != Logger.Level.NONE) {
      methodHandlerConfiguration.getLogger().logRequest(methodHandlerConfiguration.getMetadata().configKey(), methodHandlerConfiguration.getLogLevel(), request);
    }
    ...
    try {
      ...
      if (methodHandlerConfiguration.getLogLevel() != Logger.Level.NONE) {
        methodHandlerConfiguration.getLogger().logIOException(methodHandlerConfiguration.getMetadata().configKey(), methodHandlerConfiguration.getLogLevel(), e, elapsedTime(start));
      }
    ...
    return responseHandler.handleResponse(
        methodHandlerConfiguration.getMetadata().configKey(), response, methodHandlerConfiguration.getMetadata().returnType(), elapsedTime);
  }
  ...
  Options findOptions(Object[] argv) {
    if (argv == null || argv.length == 0) {
      return this.methodHandlerConfiguration.getOptions().getMethodOptions(methodHandlerConfiguration.getMetadata().method().getName());
    }
    return Stream.of(argv)
        .filter(Options.class::isInstance)
        .map(Options.class::cast)
        .findFirst()
        .orElse(this.methodHandlerConfiguration.getOptions().getMethodOptions(methodHandlerConfiguration.getMetadata().method().getName()));
  }
